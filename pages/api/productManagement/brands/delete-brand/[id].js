...
export default async (req, res) => {
    ...
    if (!tokenSession || tokenSession.exp > nowTime) {
        ...
        const update = await Brands.update({ deleted_id: user_id }, {
            where: {
                id: id
            }
        });
        if (update) {
            ...
            const deleteBrand = await Brands.destroy({
                where: {
                    id: id
                }
            });
            if (deleteBrand) {
                require('dns').lookup(require('os').hostname(), async (err, add, fam) => {
                    const insertLog = await Logs.create({
                        email: user_email,
                        action: 'Marka tablosundaki "' + brand[0].brand_name + '" isimli marka silindi.', ip_address: add
                    })
                })
