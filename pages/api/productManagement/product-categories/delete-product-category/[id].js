...
export default async (req, res) => {
    ...
    if (!tokenSession || tokenSession.exp > nowTime) {
        ...
        const update = await ProductCategories.update({ deleted_id: user_id }, {
            where: {
                id: id
            }
        });
        if (update) {
            ...
            const deleteProductCategory = await ProductCategories.destroy({
                where: {
                    id: id
                }
            });
            if (deleteProductCategory) {
                require('dns').lookup(require('os').hostname(), async (err, add, fam) => {
                    const insertLog = await Logs.create({
                        email: user_email,
                        action: 'Ürün kategori tablosundaki "' + productCategory[0].category_name + '" isimli kategori silindi.', ip_address: add
                    })
                })
