...
export default async (req, res) => {
    ...
    if (!tokenSession || tokenSession.exp > nowTime) {
        ...
        if (id != 0) {
            ...
            const update = await Categories.update({category_name: category_name}, {
                where: {
                    id: id
                }
            });
            if (update) {
                require('dns').lookup(require('os').hostname(), async (err, add, fam) => {
                    await Logs.create({
                        email: user_email,
                        action: 'Kategori tablosundaki "' + category[0].category_name + '" isimli kategori "' + category_name + '" olarak güncellendi.',
                        ip_address: add
                    })
                })
            ...
            //insert
            const insert = await Categories.create({category_name: category_name, user_id: user_id})
            if (insert) {
                require('dns').lookup(require('os').hostname(), async (err, add, fam) => {
                    const insertLog = await Logs.create({
                        email: user_email,
                        action: 'Kategori tablosuna "' + category_name + '" isimli kategori eklendi.',
                        ip_address: add
                    })
                })
