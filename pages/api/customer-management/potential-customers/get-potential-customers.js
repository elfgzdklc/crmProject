...
export default async (req, res) => {
    ...
    if (!tokenSession || tokenSession.exp > nowTime) {
        ...
        if (userLiable[0].user_liable == 1) {   //sorumlu kişilerin görebileceği müşterileri getirir.
            ...
            let meetingCountArray = [];
            for (let j = 0; j < customers.length; j++) {
                let meetingCount = await CustomerMeetings.count({
                    where: {
                        customer_id: customers[j].id
                    }
                });
                meetingCountArray.push({countMeet: meetingCount});
            }
            let allCustomers = customers.map((customer, i) => Object.assign({}, customer.dataValues, meetingCountArray[i]));
            res.json({
                total: total.length,
                data: allCustomers
            });
