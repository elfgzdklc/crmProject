...
export default async (req, res) => {
    ...
    if (!tokenSession || tokenSession.exp > nowTime) {
        ...
        Customers.hasOne(CustomerCategories, {sourceKey: 'category_id', foreignKey: 'id'});
        Customers.hasOne(CustomerContacts, {sourceKey: 'id', foreignKey: 'customer_id'});
        ...
        if (id == 0) {
            ...
            if (customer) {
                require('dns').lookup(require('os').hostname(), async (err, add, fam) => {  //ip adresi alınıyor
                    await Logs.create({                                          //log oluşturuluyor
                        ...
                        action: "Potansiyel firma tablosuna " + '"' + trade_name + '"' + " isimli firma eklendi.",
                        ip_address: add
                    });
                });
            ...
            if (customer) {
                ...
                if (customer_contact) {
                    require('dns').lookup(require('os').hostname(), async (err, add, fam) => {  //ip adresi alınıyor
                        if (old_customer_name[0].trade_name != trade_name) {
                            await Logs.create({                                          //log oluşturuluyor
                                email: session.user.email,
                                action: "Potansiyel firma tablosundaki " + '"' + old_customer_name[0].trade_name + '"' + " isimli firma " + '"' + trade_name + '"' + " olarak güncellendi.",
                                ip_address: add
                            });
                        } else {
                            await Logs.create({                                          //log oluşturuluyor
                                email: session.user.email,
                                action: "Potansiyel firma tablosundaki " + '"' + old_customer_name[0].trade_name + '"' + " isimli firma bilgilerinde güncelleme yapılmıştır.",
                                ip_address: add
                            });
                        }
                    });
