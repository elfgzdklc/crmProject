...
//     const offers = await axios.post(`${path}api/offers/get-rejected-offers`, {
//         limit: 10,
//         page: 1,
//         sortColumn: 'id',
//         sortType: 'desc',
...
function PendingOffers(props) {
    ...
    async function getPermissionDetail() {
        ...
        await axios({
            ...
            setLoading(false)
            if (response.data[0] === undefined || response.data[0].rejected_offers === 0) {
    ...
    async function getOffers() {
        ...
        await axios({
            method: 'POST',
            url: '/api/offers/get-rejected-offers',
            headers: {
                'Content-Type': 'application/json',
                AuthToken: props.token
            },
    ...
    return (
        <div>
            <Title title="Reddedilmiş Teklifler"/>
            ...
            <Modal size="xl" show={showDetail} onHide={handleCloseDetail} keyboard={false} aria-labelledby="example-modal-sizes-title-xl">
                ...
                <Modal.Footer>
                    ...
                    <button type="button" className="p-2 btn-sm">
                        <i className="fal fa-times-circle"></i> Reddedildi
                    </button>
                </Modal.Footer>
            </Modal>
        </div>
